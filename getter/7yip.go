...
// 获取7yip
func Get7yip() (result []*models.IP) {
	fmt.Println("采集IP - 7yip")
	for i := 1; i < 10; i++ {
		time.Sleep(5 * time.Second)

		url := "https://www.7yip.cn/free/?action=china&page=" + strconv.Itoa(i)
		...
		dom.Find(".table-striped tbody tr").Each(func(i int, context *goquery.Selection) {
			...
			if ssssss == "高匿" && lib.QueryIp(ip, port) == false {
				//lib.Insert(ip, port, strings.ToLower(pop), area)
